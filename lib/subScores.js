...
// funded by buybacks) while being exceptionally strong on the third — and that should visibly
// pull the category up, not get buried under two ratios that don't capture it. Equal-weighted
// across whichever of the three are available for this ticker (FCF margin isn't always reported,
// e.g. for banks — see lib/sectorBenchmarks.js), not just averaged in as an afterthought.
function buildFinancialHealth({ debtEquityRaw, currentRatioRaw, fcfMarginRaw }, sector) {
  ...
  const parts = [deScore, crScore, fcfScore].filter(v => v != null);
  if (!parts.length) return null;
  const avg = parts.reduce((s, v) => s + v, 0) / parts.length;
