...
// Debt/equity and current ratio are both balance-sheet SNAPSHOTS — they can't tell the
// difference between "distressed" leverage and "strategic" leverage a company can comfortably
// service. FCF margin is the missing piece: whether the business actually generates enough real
// cash, relative to revenue, to cover that debt. A company can look weak on the first two ratios
// (Apple's debt/equity and current ratio both run worse than the Technology sector median,
// funded by buybacks) while being exceptionally strong on the third.
//
// An earlier version of this function equal-weighted the three metrics — but with D/E and current
// ratio landing deep in "poor" territory (as they genuinely do for Apple) and FCF margin landing
// deep in "clear strength" territory, a 1-in-3 average lets the two weak snapshots outvote the one
// strong cash-generation signal 2-to-1, which never actually let FCF margin "pull the category
// up" the way this comment always claimed it should — verified against live AAPL data: D/E=14,
// CR=20, FCF=80 on their own 0-100 scales average to just 38 (4/10), not the "clear strength
// visibly pulling up" outcome the comment promised.
//
// Fixed two ways: (1) FCF margin now carries HALF the weight (D/E and current ratio split the
// other half), so cash generation is the dominant signal rather than one of three equal votes;
// (2) when FCF margin is independently a "clear strength" (score >= 80, the same threshold
// qualitativeTag() already uses), it sets a FLOOR of 70 the blended score can't fall below — this
// is what actually implements "shouldn't get buried," rather than just weighting it more heavily.
// The floor is safe for genuinely distressed companies: it can only ever fire when FCF margin
// itself is independently strong, and real distress means FCF margin is weak or negative too, so
// the floor condition simply never triggers for a company that's actually in trouble.
const FINANCIAL_HEALTH_WEIGHTS = { debtEquity: 0.25, currentRatio: 0.25, fcfMargin: 0.50 };
const STRONG_FCF_FLOOR_TRIGGER = 80;  // same "clear strength" threshold as qualitativeTag()
const STRONG_FCF_FLOOR_SCORE = 70;    // floor applied once that trigger is met

function buildFinancialHealth({ debtEquityRaw, currentRatioRaw, fcfMarginRaw }, sector) {
  const b = sector.benchmarks;
  ...
  const fcfScore = scoreAgainstBenchmark(fcfMarginRaw, b.fcfMargin, true);

  const weighted = [
    { score: deScore, weight: FINANCIAL_HEALTH_WEIGHTS.debtEquity },
    { score: crScore, weight: FINANCIAL_HEALTH_WEIGHTS.currentRatio },
    { score: fcfScore, weight: FINANCIAL_HEALTH_WEIGHTS.fcfMargin }
  ].filter(p => p.score != null);
  if (!weighted.length) return null;
  const totalWeight = weighted.reduce((s, p) => s + p.weight, 0);
  const rawBlend = weighted.reduce((s, p) => s + p.score * p.weight, 0) / totalWeight;

  const strongFcf = fcfScore != null && fcfScore >= STRONG_FCF_FLOOR_TRIGGER;
  const avg = strongFcf ? Math.max(rawBlend, STRONG_FCF_FLOOR_SCORE) : rawBlend;
  ...
  if (fcfScore != null) reasons.push(describeMetric('Free cash flow margin', fcfMarginRaw, b.fcfMargin, fcfScore, sector.label, v => pct(num1(v))));
  if (strongFcf && rawBlend < STRONG_FCF_FLOOR_SCORE) {
    reasons.push('strong free cash flow generation keeps this category from scoring lower despite the balance-sheet ratios above');
  }
