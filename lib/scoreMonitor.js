...
import { getPeerComparisonLine } from './peerAlertComparison.js';
...
// `suppressedFlagIds` (optional, defaults to none) excludes specific flag ids from the individual
// red_flag notification — used when lib/crossHoldingFlags.js has already folded a flag id into a
// consolidated cross-holding notification for this user this run, so the same signal doesn't also
// ping individually for this one ticker.
export function decideAlerts(row, fresh, suppressedFlagIds = new Set()) {
  const notifications = [];
  ...
  const triggeredFlags = Array.isArray(fresh?.redFlags)
    ? fresh.redFlags.filter(f => (f?.severity === 'high' || f?.severity === 'medium') && !suppressedFlagIds.has(f.id))
    : [];
  if (triggeredFlags.length) {
    ...
    // Static "what to check next" text (lib/redFlagActions.js), already attached to `primary` by
    // lib/redFlags.js's computeRedFlags() — a guaranteed-instant baseline appended here, ahead of
    // the richer Groq-generated checklist below (which is best-effort and can fail/degrade).
    const flagNames = triggeredFlags.map(f => f.name).join(', ');
    const body = primary?.action ? `${flagNames}\n\nQuick check: ${primary.action}` : flagNames;
    notifications.push({
      type: 'red_flag',
...
// Fetches a factual peer comparison sentence for a red_flag notification and appends it to the
// body — best-effort: any failure (no peers, no Finnhub data) just leaves the body unchanged.
// Informational only, never personalized recommendations, consistent with APEX's regulatory stance.
async function withPeerComparison(n, fresh, ticker, finnhubKey) {
  if (n.type !== 'red_flag' || !finnhubKey) return n.body;
  // primary flag is the most severe one — same selection rule as decideAlerts() uses for checklist.
  const triggeredFlags = Array.isArray(fresh?.redFlags)
    ? fresh.redFlags.filter(f => f?.severity === 'high' || f?.severity === 'medium')
    : [];
  if (!triggeredFlags.length) return n.body;
  const primary = triggeredFlags.reduce(
    (worst, f) => (!worst || (f.severity === 'high' && worst.severity !== 'high')) ? f : worst,
    null
  );
  // Attach ticker to the fresh object so getPeerComparisonLine can include it in the sentence.
  const freshWithTicker = { ...fresh, ticker };
  const line = await getPeerComparisonLine(freshWithTicker, primary.id, finnhubKey);
  if (!line) return n.body;
  return `${n.body}\n\n${line}`;
}
...
// numbers. `suppressedFlagsByTicker` (optional, defaults to none) is the Map produced by
// lib/crossHoldingFlags.js's runCrossHoldingFlagMonitor — flag ids already folded into a
// consolidated cross-holding notification for a given ticker's user this run are excluded from
// that ticker's individual red_flag notification, so the same signal doesn't ping twice.
// `finnhubKey` (optional) enables peer comparison lines on red_flag notifications — one lightweight
// Finnhub /stock/metric call per alert fired; omit to skip peer comparison (alerts still fire).
export async function runScoreMonitor(serviceRoleKey, supabaseUrl, table, rows, freshByTicker, suppressedFlagsByTicker = new Map(), finnhubKey = null) {
  ...
  for (const row of rows) {
    ...
    const suppressed = suppressedFlagsByTicker.get(row.ticker) || new Set();
    const { notifications, baselineUpdate } = decideAlerts(row, fresh, suppressed);
    ...
    for (const n of notifications) {
      ...
      // Peer comparison is applied before the checklist so it sits immediately after the flag
      // summary, with the checklist items following — most urgent context first.
      const bodyWithPeer = await withPeerComparison(n, fresh, row.ticker, finnhubKey);
      const nWithPeer = { ...n, body: bodyWithPeer };
      const body = await withChecklist(nWithPeer);
      const created = await createNotification(serviceRoleKey, {
        userId: row.user_id,
        type: nWithPeer.type,
        ticker: row.ticker,
        companyName: row.company_name,
        title: nWithPeer.title,
        body
      });
      if (created) notified++;
    }
