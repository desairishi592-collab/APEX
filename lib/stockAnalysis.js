...
import { resolveSector, scoreAgainstBenchmark } from './sectorBenchmarks.js';
import { computeSubScores, scoreMomentum } from './subScores.js';
...
//
// Every metric here except 1-year return is scored against `sector.benchmarks` (see
// lib/sectorBenchmarks.js) rather than a flat universal cutoff — "normal" leverage, margins,
// and volatility vary a lot by industry (e.g. banks/utilities/REITs run high D/E by design;
// a flat threshold would misjudge them as risky purely for being in a capital-intensive sector).
function computeDeterministicQualityScore({ profitMarginRaw, roeRaw, debtEquityRaw, betaRaw, yearReturnRaw }, sector) {
  const b = sector.benchmarks;
  const parts = [];
  const profitMarginScore = scoreAgainstBenchmark(profitMarginRaw, b.profitMargin, true);
  if (profitMarginScore != null) parts.push({ v: profitMarginScore, w: 0.25 });

  const roeScore = scoreAgainstBenchmark(roeRaw, b.roe, true);
  if (roeScore != null) parts.push({ v: roeScore, w: 0.25 });

  const debtEquityScore = scoreAgainstBenchmark(debtEquityRaw, b.debtEquity, false);
  // Weighted lightly even sector-relative, since leverage norms still vary within a sector.
  if (debtEquityScore != null) parts.push({ v: debtEquityScore, w: 0.15 });

  const betaScore = scoreAgainstBenchmark(betaRaw, b.beta, false);
  if (betaScore != null) parts.push({ v: betaScore, w: 0.10 });

  // Price return isn't sector-relative — a good return is a good return in any industry.
  // Shared with the Momentum sub-score (lib/subScores.js) so the two don't drift apart.
  const momentumScore = scoreMomentum(yearReturnRaw);
  if (momentumScore != null) parts.push({ v: momentumScore, w: 0.25 });
...
- Return ONLY the JSON object.`;
  ...
  return await callGroqForJson(prompt, (parsed) => {
    ...
    const qualityScore = computeDeterministicQualityScore({ profitMarginRaw, roeRaw, debtEquityRaw, betaRaw, yearReturnRaw }, sector);
    ...
    parsed.rawMetrics = { peRaw, debtEquityRaw, currentRatioRaw, roeRaw, profitMarginRaw, yearReturnRaw };
    ...
    // Explainable breakdown of the overall score into per-category sub-scores with plain-
    // English reasons — additive, doesn't change `score` itself. Reuses the same sector
    // benchmark data and raw metrics already computed above (see lib/subScores.js), and rolls
    // up the redFlags array just computed rather than re-deriving anything.
    parsed.subScores = computeSubScores({ profitMarginRaw, roeRaw, debtEquityRaw, betaRaw, currentRatioRaw, peRaw, yearReturnRaw }, sector, parsed.redFlags);
  });
}
