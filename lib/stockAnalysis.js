...
import { computeRedFlags, getLatestSeriesValue } from './redFlags.js';
...
export async function handleStockAnalysis(ticker, companyName) {
  ...
  // Latest-quarter FCF margin — same series/candidate-keys lib/redFlags.js already reads for its
  // FCF-decline check, reused here (not a second Finnhub call) so Financial Health can weigh real
  // cash-generation capacity alongside the balance-sheet snapshot ratios above. Feeds the sub-score
  // only; redFlags.js keeps computing its own trend check independently from the same series.
  const fcfMarginRaw = getLatestSeriesValue(metrics, ['fcfMargin', 'focfMargin', 'freeCashFlowMargin']);
