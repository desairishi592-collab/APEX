...
export function computeSectorConcentration(tickerSectorPairs) {
  ...
  return {
    ...
    // Additive: the full sector -> tickers breakdown, not just whichever sector happens to be
    // dominant overall — decideScanTimeConcentrationNudge() below needs to look up ONE specific
    // sector's count (the sector of the ticker just scanned), which may not be the dominant one.
    bySector
  };
}
...
// Pure — no I/O. Scan-time counterpart to decideConcentrationAlert() above: instead of the weekly
// cron's percentage-of-total threshold, this checks what adding ONE specific new ticker (the one
// just scanned) would do to that ticker's own sector count — "this would be your Nth <sector>
// holding" — since a real-time nudge should react to the exact holding just added, not wait for a
// dominant-share crossing across the whole portfolio. Reuses computeSectorConcentration() for the
// aggregation and MIN_HOLDINGS_FOR_CONCENTRATION_ALERT for the same "too few to be meaningful"
// floor, rather than duplicating either. Returns null when the ticker is already tracked (nothing
// hypothetical to check), has no resolved sector, or wouldn't reach that floor.
export function decideScanTimeConcentrationNudge(existingTickerSectorPairs, newTicker, newSector) {
  if (!newSector) return null;
  if (existingTickerSectorPairs.some(p => p.ticker === newTicker)) return null;

  const hypothetical = [...existingTickerSectorPairs, { ticker: newTicker, sector: newSector }];
  const concentration = computeSectorConcentration(hypothetical);
  if (!concentration) return null;

  const sectorTickers = concentration.bySector[newSector] || [];
  const count = sectorTickers.length;
  if (count < MIN_HOLDINGS_FOR_CONCENTRATION_ALERT) return null;

  return {
    sector: newSector,
    count,
    message: `This would be your ${ordinal(count)} ${newSector}-sector holding.`
  };
}
