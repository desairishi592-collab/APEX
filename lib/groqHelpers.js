...
async function requestJsonCompletion(url, apiKey, model, prompt, timeoutMs, extraBody) {
  return fetch(url, {
    method: 'POST',
    headers: {
...
// Pins the OpenRouter call to Cerebras's backend for gpt-oss-120b specifically — production
// benchmarks show throughput on this model varies wildly by backend OpenRouter can route to
// (Cerebras ~1,667 tok/s vs. other backends under 200 tok/s), and the OPENROUTER_TIMEOUT_MS
// bump above was necessary precisely because default routing was landing somewhere slow.
// allow_fallbacks stays true so a Cerebras-specific outage degrades to OpenRouter's normal
// routing instead of hard-failing the fallback entirely.
...
async function tryOpenRouterFallback(prompt) {
  ...
  try {
    const res = await requestJsonCompletion('https://openrouter.ai/api/v1/chat/completions', apiKey, OPENROUTER_MODEL, prompt, OPENROUTER_TIMEOUT_MS, { provider: OPENROUTER_PROVIDER_PREFERENCE });
    if (!res.ok) {
