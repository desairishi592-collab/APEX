...
async function tryOpenRouterFallback(prompt) {
  ...
  try {
    const res = await requestJsonCompletion('https://openrouter.ai/api/v1/chat/completions', apiKey, OPENROUTER_MODEL, prompt);
    if (!res.ok) {
    ...
    console.error('OpenRouter fallback request threw:', e.message);
    return null;
  }
}
